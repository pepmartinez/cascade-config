...
describe('cascade-config test', function () {
  describe('plain, nontemplated', function () {
    ...
    before(function (done) {
      MongoClient.connect(mongodb_url, function (err, db) {
        var collection = db.collection(mongodb_coll);
    ...
    it('does read and merge from mongodb ok', function (done) {
      ...
      mconf
        .mongodb({ url: mongodb_url, coll: mongodb_coll, id: '666' })
        .mongodb({ url: mongodb_url, coll: mongodb_coll, id: 'zxcvbnm' })
        .mongodb({ url: mongodb_url, coll: mongodb_coll, id: 'qwerty' })
        .done(function (err, cfg) {
    ...
    it('does return empty object on nonexistent mongodb', function (done) {
      ...
      mconf
        .mongodb({ url: mongodb_url, coll: mongodb_coll, id: 'nonexistent' })
        .done(function (err, cfg) {
          cfg.should.eql({});
          done();
        })
  ...
  describe('templated', function () {
    var mongodb_url = 'mongodb://localhost:27017/db_development_';
    ...
    it('merges from templatized mongo ok', function (done) {
      ...
      mconf
        .obj ({ggg: 3, gamma: {a: 6}})
        .mongodb({ url: 'mongodb://localhost:27017/db_{env}_', coll: 'mconf_{env}_', id: 'id-{env}-{gamma.a}'})
        .done(function (err, cfg) {
